...
function getAirPollution(lat,lng){
    ...
    .then(function(response){
        ...
        Weather2.innerText= `   Tomorrow :  ${tomorrowForecast}   `
        Weather3.innerText= `  TDAT :  ${theDayAfterTomorrowForecast}`
        WeIcon.innerHTML=`<img src=http://openweathermap.org/img/wn/${icon[0]}@2x.png height=45 width=42> `
        WeIcon2.innerHTML=`<img src=http://openweathermap.org/img/wn/${icon[1]}@2x.png height=45 width=42>`
        WeIcon3.innerHTML=`<img src=http://openweathermap.org/img/wn/${icon[2]}@2x.png height=45 width=42>`
...
function loadcoords(){
    ...
    if(loadedCoords===null){
        ...
        const parsedCoords=JSON.parse(loadedCoords); // parse the json and create the obj
